...
for k, (title, tag, desc) in IMGS.items():
    img_path = f"/app/static/{k}.png"
    gh_path = f"https://cdn.jsdelivr.net/gh/Dexter-1611/Codify_AI@main/static/{k}.png"
    entries.append('{\"title\":\"' + title + '\",\"tag\":\"' + tag + '\",\"desc\":\"' + desc + '\",\"img_path\":\"' + img_path + '\",\"gh_path\":\"' + gh_path + '\"}')
...
#stage {{ position:relative; width:100%; height:100%; display:flex; align-items:center; justify-content:center; isolation:isolate; }}
#heading {{ position:absolute;top:24px;left:0;right:0;text-align:center; z-index:5;pointer-events:none; }}
.ey {{ font-size:.62rem;letter-spacing:.22em;text-transform:uppercase;color:#D4AF65; margin-bottom:10px;display:flex;align-items:center;justify-content:center;gap:10px; }}
...
.gold {{ background:linear-gradient(105deg,#D4AF65,#EDD98A,#FFFAED,#EDD98A,#D4AF65);background-size:200% auto; -webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;animation:gs 4s linear infinite; }}
@keyframes gs {{ 0%{{background-position:-200% center}}100%{{background-position:200% center}} }}
#grid {{ display:grid; grid-template-columns:1fr 1.6fr 1.6fr 1fr; grid-template-rows:200px 200px 110px; gap:14px;width:min(1300px,92vw); padding-top:100px; position:relative; }}
.bc {{ position:relative;border-radius:16px;overflow:hidden; border:1px solid rgba(212,175,101,.18);background:#0D0D0D; box-shadow:0 4px 24px rgba(0,0,0,.7);cursor:pointer; transition:border-color 0.3s ease; will-change:transform; transform:translate3d(0,0,0); }}
...
#bC {{ grid-column:2/4;grid-row:2/4;z-index:10; border-color:rgba(212,175,101,.3);box-shadow:0 8px 48px rgba(0,0,0,.85);overflow:hidden; }}
...
.nb {{ width:40px;height:40px;border-radius:50%;background:rgba(20,18,14,.6);border:1px solid rgba(212,175,101,.2); backdrop-filter:blur(8px);display:flex;align-items:center;justify-content:center;cursor:pointer; transition:all .3s ease;pointer-events:auto; }}
...
<body>
<div id="stage"><div id="heading"><div class="ey">Codify AI &mdash; The Obsidian Assembly</div><h1>Craft <em>with</em> <span class="gold">Intelligence</span></h1></div><div id="grid"><div class="bc" id="bL" data-idx="0"><img id="iL" src="https://cdn.jsdelivr.net/gh/Dexter-1611/Codify_AI@main/static/neural.png" alt=""><div class="ov"></div><div class="lb"><div class="lt" id="tL">AI Architecture</div><div class="ln" id="nL">Neural Synthesis</div></div></div><div class="bc" id="bT1" data-idx="1"><img id="iT1" src="https://cdn.jsdelivr.net/gh/Dexter-1611/Codify_AI@main/static/code_float.png" alt=""><div class="ov"></div><div class="lb"><div class="lt" id="tT1">Language Engine</div><div class="ln" id="nT1">Code Generation</div></div></div><div class="bc" id="bT2" data-idx="2"><img id="iT2" src="https://cdn.jsdelivr.net/gh/Dexter-1611/Codify_AI@main/static/ai_hand.png" alt=""><div class="ov"></div><div class="lb"><div class="lt" id="tT2">Interface Layer</div><div class="ln" id="nT2">Human &times; Machine</div></div></div><div class="bc" id="bT3" data-idx="3"><img id="iT3" src="https://cdn.jsdelivr.net/gh/Dexter-1611/Codify_AI@main/static/data_stream.png" alt=""><div class="ov"></div><div class="lb"><div class="lt" id="tT3">Stream Processing</div><div class="ln" id="nT3">Data Intelligence</div></div></div><div class="bc" id="bC" data-idx="-1">
<video id="vC" autoplay loop muted playsinline style="width:100%; height:100%; object-fit:cover; transform:scale(0.92); transform-origin:center center; will-change:transform;">
    <source src="https://cdn.jsdelivr.net/gh/Dexter-1611/Codify_AI@main/static/ai_core_comp.mp4" type="video/mp4">
    <source src="/app/static/ai_core_comp.mp4" type="video/mp4">
</video><div class="ov"></div><div class="cbot"><div><div class="ctag" id="ctag">Codify Core</div><div class="cttl" id="cttl">AI Consciousness</div></div></div><div class="nav"><button class="nb prev" id="bP"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"/></svg></button><button class="nb next" id="bN"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg></button></div></div><div class="bc" id="bR" data-idx="4"><img id="iR" src="https://cdn.jsdelivr.net/gh/Dexter-1611/Codify_AI@main/static/terminal.png" alt=""><div class="ov"></div><div class="lb"><div class="lt" id="tR">CLI Powerhouse</div><div class="ln" id="nR">Terminal Mastery</div></div></div></div></div><div id="infoModal"><div class="modal-content"><button class="modal-close" id="modalClose">&times;</button><div id="modTag">Tag</div><div id="modTitle">Title</div><div id="modDesc">Description goes here...</div></div></div>
<script>
(function(){{
'use strict';

// 1. Resolve Parent URL via Streamlit Query Params or Referrer
let base = "";
try {{
    const urlParams = new URLSearchParams(window.location.search);
    let stUrl = urlParams.get('streamlitUrl');
    if (stUrl) {{
        let ref = new URL(stUrl);
        base = ref.origin + ref.pathname.replace(/\/$/, "");
...
// 2. Fallback logic for video (if jsDelivr fails) is handled by <source> tags
let vC = document.getElementById('vC');
vC.onerror = function() {{
    console.log("Video source failed, attempting fallback...");
}};
...
function sync(ci, isFirst){{
  ...
  refs.forEach(function(r,k){{
    var p=P[m[k]]; var el=G(r.im); 
    if (isFirst) {{
        el.onerror = function() {{ el.src = base + p.img_path; }};
        G(r.t).innerHTML = p.tag; G(r.n).innerHTML = p.title; G(r.c).setAttribute('data-idx', m[k]);
        return;
    }}
    el.style.opacity='0';
    setTimeout(function(){{
        // Prioritize jsDelivr CDN for lightning fast global loads, fallback to Streamlit static
        el.onerror = function() {{ el.src = base + p.img_path; }};
        el.src = p.gh_path; 
        el.style.opacity='1';
    }},150);
...
function adv(d){{cur=w(cur+d);sync(cur, false);}}
...
out = (
    ...
    '    """Render Codify AI bento gallery with jsDelivr ultra-fast CDN and compressed video"""\n'
    "    components.html(BENTO_CINEMA_HTML, height=652, scrolling=False)\n"
)

with open('bento_cinema.py', 'w', encoding='utf-8') as f:
    f.write(out)

print('bento_cinema.py updated with jsDelivr CDN links and compressed video payload!')
